...
//  ********** Library C:\Users\Carlos\dart\Calculadora\Calculadora **************
// ********** Code for Logic **************
function Logic() {
...
Logic.prototype.run = function() {
  var $this = this; // closure support
  this.elemento = get$$document().query("#divResultado");
  this.cantidad = (0.0);
  this.tmpCantidad = (0.0);
  this.operacion = "";
  this.tmpOperacion = "";
  this.digitacion = false;
  var $$list = get$$document().queryAll(".btnCalculadora");
  for (var $$i = $$list.iterator(); $$i.hasNext(); ) {
    var e = $$i.next();
    e.get$on().get$click().add$1((function (ele) {
      return $this.calculateFunction(ele);
    })
    );
  }
}
Logic.prototype.calculateFunction = function(e) {
  var valor = e.get$target().get$value();
  var isNumber = (-1);
  try {
    isNumber = Math.parseInt(valor);
  } catch (e1) {
    e1 = _toDartException(e1);
    if (!(e1 instanceof BadNumberFormatException)) throw e1;
  }
  if (isNumber == (-1)) {
    this.operacion = valor;
    this.performOperation(valor);
  }
  else {
    this.setValue(valor);
  }
}
Logic.prototype.setValue = function(valor) {
  if ($eq$(this.digitacion, true)) {
    this.elemento.set$innerHTML($add$($add$(this.elemento.get$innerHTML(), ""), valor));
  }
  else {
    if ($ne$(valor, "0")) {
      this.elemento.set$innerHTML(valor);
      this.digitacion = true;
    }
  }
  this.cantidad = Math.parseDouble(this.elemento.get$innerHTML());
}
Logic.prototype.performOperation = function(valor) {
  if (this.operacion == "AC") {
    this.operacion = "";
    this.tmpOperacion = "";
    this.cantidad = (0.0);
    this.tmpCantidad = (0.0);
    this.digitacion = false;
    this.elemento.set$innerHTML("0");
  }
  else if (this.operacion == "Sqrt") {
    this.cantidad = Math.sqrt(this.cantidad);
  }
  else {
    this.doOperation();
    this.tmpCantidad = this.cantidad;
    this.tmpOperacion = this.operacion;
  }
  this.elemento.set$innerHTML("" + this.cantidad);
  this.digitacion = false;
}
Logic.prototype.doOperation = function() {
  if (this.tmpOperacion == "+") {
    this.cantidad = this.tmpCantidad + this.cantidad;
  }
